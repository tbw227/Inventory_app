...
import { mediaUrl, isProtectedUploadPath, isSafeImageSrc } from '../../utils/mediaUrl'
...
export default function AuthedImg({ src, alt = '', className = '', loading = 'lazy', ...rest }) {
  ...
  if (!isProtectedUploadPath(src)) {
    const passThroughSrc = mediaUrl(src)
    if (!isSafeImageSrc(passThroughSrc)) return null
    return <img src={passThroughSrc} alt={alt} className={className} loading={loading} {...rest} />
  }
  ...
  if (!isSafeImageSrc(blobUrl)) return null
